...
const CoinInformation = () => {
  ...
  const _caculateCoinOther = (item: CoinAllocation) => {
    let rate = round((item?.usdVolume / totalVolume) * 100, 2);
  ...
  const NativeToken = useMemo(() => {
    ...
    const NameElement = (
      <div className='ml-2 text-grey-700 text-base font-jsb leading-normal'>
        {coinsData?.nativeCoin?.name}
      </div>
    );

    if (!coinsData?.nativeCoin?.logo && coinsData?.nativeCoin?.name)
      return NameElement;
  ...
  return (
    <div className='w-full bg-white mx-auto'>
      <Modal
        title={
          <div className='font-bold font-jb text-2xl'>Token Allocation Other</div>
        }
        open={isModalOpen}
